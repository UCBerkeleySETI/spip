...
class UWBArchiverDaemon(Daemon):
      ...
      for proc_type in self.proc_types:
        ...
        for beam in self.beams:
          ...
          if self.quit_event.isSet():
            self.log(1, "UWBArchiverDaemon::main quit_event true [1]")
            continue
          ...
          if not os.path.exists(send_dir):
            self.log(-1, "send_dir [" + send_dir + "] did not exist")
          ...
          if not os.path.exists(junk_dir):
            self.log(-1, "sent_dir [" + junk_dir + "] did not exist")
            os.makedirs(junk_dir, 0755)

          # look for observations that have been completed and have / BEAM / utc / source / CFREQ
          self.log(2, "UWBArchiverDaemon::main looking for obs.finished in " + send_dir + "/<UTC>/<SOURCE>/" + self.cfreq)
          ...
          for path in fin_files:
            ...
            if self.quit_event.isSet():
              self.log(1, "UWBArchiverDaemon::main quit_event true [2]")
              continue
            ...
            if os.path.exists (header_file):
              ...
              self.log (2, "UWBArchiverDaemon::main utc=" + utc + " source=" + source + " pid=" + header["PID"])
              if header["PID"] == "P999":
                transfer = False

            if transfer:
            
              self.log (2, "UWBArchiverDaemon::main get_rsync_from_stream (" + str(self.id) + ")")
              (username, server, module) = self.get_rsync_from_stream (self.id)
              self.log (2, "UWBArchiverDaemon::main rsync stream=" + str(self.id)+ " user=" + username + " server=" + server + " module=" + module)
              ...
              if rval:
                ...
                if rval:
                  transfer_success = False
                  self.log (-1, "failed to transfer " + completed_subdir + "/obs.finished")
                ...
                if transfer_success:
                  ...
                  try:
                    ...
                    self.log (2, "UWBArchiverDaemon::main " + str(e))
                  ...
                  self.log (1, proc_type + " " + utc_source + "/" + cfreq + " transferred to " + module + ": " + transfer_rate)
                else:
                  self.log (1, proc_type + " " + utc_source + "/" + cfreq + " failed to transfer")
            else:
              ...
              self.clean_utc_source_dir (send_dir + "/" + utc_source)
              self.log (1, proc_type + " " + utc_source + "/" + cfreq + " junked")
  ...
  def clean_utc_source_dir (self, source_dir):
    ...
    self.log (2, "UWBArchiverDaemon::clean_utc_source_dir cleaning " + source_dir)
      ...
      if subdir_count == 0:
        try:
          ...
          try:
            ...
            self.log (1, "UWBArchiverDaemon::main clean_utc_source_dir os.rmdir(" + utc_dir + ") " + str(e))
        except OSError, e:
          self.log (1, "UWBArchiverDaemon::main clean_utc_source_dir os.rmdir(" + source_dir + ") " + str(e))
