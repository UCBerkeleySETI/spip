...
int main(int argc, char *argv[]) try
{
  ...
  if (verbose)
    cerr << "bpsr_dbstats: creating DataBlockStats with " << key << endl;
  dbstats = new spip::DataBlockStats (key.c_str());

  dbstats->set_verbosity(verbose > 0);
  dbstats->set_block_format (new spip::BlockFormatBPSR());
  ...
  if (control_port > 0)
  {
    cerr << "bpsr_dbstats: start_control_thread (" << control_port << ")" << endl;
    ...
    {
      if (verbose)
        cerr << "bpsr_dbstats: monitor()" << endl;
      keep_monitoring = dbstats->monitor(stats_dir, stream);
      if (verbose)
        cerr << "bpsr_dbstats: monitor returned" << endl;
    }
  }
  else
  {
    cerr << "bpsr_dbstats: calling monitor" << endl;
...
void usage() 
{
  cout << "bpsr_dbstats [options] config\n"
