...
int main(int argc, char *argv[])
{
  ...
  try 
  {
    ...
    udprecv->set_format (new spip::UDPFormatBPSR());
    ...
    if (udprecv->verbose)
      cerr << "bpsr_udprecv: allocating runtime resources" << endl;
    udprecv->prepare ();

    if (udprecv->verbose)
      cerr << "bpsr_udprecv: starting stats thread" << endl;
    pthread_t stats_thread_id;
    int rval = pthread_create (&stats_thread_id, 0, stats_thread, (void *) recv);
    if (rval != 0)
    {
      cerr << "bpsr_udprecv: failed to start stats thread" << endl;
      return (EXIT_FAILURE);
    }

    if (udprecv->verbose)
      cerr << "bpsr_udprecv: receiving" << endl;
    udprecv->receive ();

    quit_threads = 1;

    if (udprecv->verbose)
      cerr << "bpsr_udprecv: joining stats_thread" << endl;
    void * result;
  ...
  {
    cerr << "bpsr_udprecv: ERROR: " << exc.what() << endl;
    return -1;
  }
...
void usage() 
{
  cout << "bpsr_udprecv [options] config\n"
