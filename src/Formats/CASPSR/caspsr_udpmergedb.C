...
int main(int argc, char *argv[])
{
  ...
  try
  {
    ...
    udpmergedb->set_formats (new spip::UDPFormatCASPSR(), new spip::UDPFormatCASPSR());
    ...
    if (verbose)
      cerr << "caspsr_udpmergedb: configuring using fixed config" << endl;
    udpmergedb->configure (config.raw());

    if (control_port > 0)
    {
      // open a listening socket for observation parameters
      cerr << "caspsr_udpmergedb: start_control_thread (" << control_port << ")" << endl;
      udpmergedb->start_control_thread (control_port);

      while (!quit_threads)
      {
        if (verbose)
          cerr << "caspsr_udpmergedb: receiving" << endl;
        udpmergedb->start_threads (core1, core2);
        udpmergedb->join_threads ();
        cerr << "caspsr_udpmergedb: receive returned" << endl;
    ...
    else
    {
      ...
      if (verbose)
        cerr << "caspsr_udpmergedb: waiting for meta-data" << endl;
      if (udpmergedb->open ())
      {
        cerr << "caspsr_udpmergedb: issuing start command" << endl;
        udpmergedb->set_control_cmd (spip::Start);
      }
      else
      {
        if (verbose)
          cerr << "caspsr_udpmergedb: failed to open data stream" << endl;
      }
      //if (nsecs > 0)
      //  udpmergedb->set_control_cmd (spip::Stop, nsecs);

      if (verbose)
        cerr << "caspsr_udpmergedb: udpmergedb->join_threads()" << endl;
      udpmergedb->join_threads ();
      cerr << "caspsr_udpmergedb: exiting" << endl;
    }
  ...
  {
    cerr << "caspsr_udpmergedb: ERROR: " << exc.what() << endl;
    return -1;
  }
...
void usage() 
{
  cout << "caspsr_udpmergedb [options] header\n"
