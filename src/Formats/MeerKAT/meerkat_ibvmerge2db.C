...
spip::IBVReceiveMerge2DB * ibvmerge2db;
char quit_threads = 0;
...
int main(int argc, char *argv[])
{
  ...
  try
  {
    ...
    if (format.compare("spead") == 0)
      ...
      ibvmerge2db->set_formats (new spip::UDPFormatMeerKATSPEAD1k(), new spip::UDPFormatMeerKATSPEAD1k());
    else if (format.compare("spead2k") == 0)
      ibvmerge2db->set_formats (new spip::UDPFormatMeerKATSPEAD2k(), new spip::UDPFormatMeerKATSPEAD2k());
    else
    {
    ...
    if (verbose)
      cerr << "meerkat_ibvmerge2db: configuring using fixed config" << endl;
    ibvmerge2db->configure (config.raw());

    if (control_port > 0)
    {
      // open a listening socket for observation parameters
      cerr << "meerkat_ibvmerge2db: start_control_thread (" << control_port << ")" << endl;
      ibvmerge2db->start_control_thread (control_port);

      while (!quit_threads)
      {
        if (verbose)
          cerr << "meerkat_ibvmerge2db: starting threads" << endl;
        ibvmerge2db->start_threads (core1, core2);
        ibvmerge2db->join_threads ();
        if (verbose)
          cerr << "meerkat_ibvmerge2db: threads ended" << endl;
        ibvmerge2db->set_control_cmd (spip::None);
      }
      ibvmerge2db->stop_control_thread ();
    }
    else
    {
      if (verbose)
        cerr << "meerkat_ibvmerge2db: receiving" << endl;
      ibvmerge2db->start_threads (core1, core2);

      if (verbose)
        cerr << "meerkat_ibvmerge2db: opening data block" << endl;
      if (ibvmerge2db->open ())
      {
        cerr << "meerkat_ibvmerge2db: issuing start command" << endl;
        ibvmerge2db->set_control_cmd (spip::Start);
      }
      else
      {
        if (verbose)
          cerr << "meerkat_ibvmerge2db: failed to open data stream" << endl;
      }
      ibvmerge2db->join_threads ();
    }
  ...
  {
    cerr << "meerkat_ibvmerge2db: ERROR: " << exc.what() << endl;
    return -1;
  }
...
void usage() 
{
  cout << "meerkat_ibvmerge2db [options] header key1 key2\n"
...
void signal_handler(int signalValue)
{
  ...
  quit_threads = 1;
  ibvmerge2db->set_control_cmd (spip::Quit);
}
