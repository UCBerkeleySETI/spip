...
int main(int argc, char *argv[])
{
  ...
  simdb->set_format (new spip::UDPFormatMeerKATSimple());
  ...
  if (verbose)
    cerr << "meerkat_simdb: configuring based on header" << endl;
  simdb->configure (config.raw());

  if (verbose)
    cerr << "meerkat_simdb: preparing" << endl;
  simdb->prepare ();
  ...
  {
    if (verbose)
      cerr << "meerkat_simdb: starting stats thread" << endl;
    simdb->start_stats_thread();

    if (verbose)
      cerr << "meerkat_simdb: writing header to data block" << endl;
    simdb->open (config.raw());

    if (verbose)
      cerr << "meerkat_simdb: issuing start command" << endl;
    simdb->start_transmit();

    if (verbose)
      cerr << "meerkat_simdb: calling receive" << endl;
    simdb->generate (transmission_time);

    if (verbose)
      cerr << "meerkat_simdb: stopping stats thread" << endl;
...
void usage() 
{
  cout << "meerkat_simdb [options] config_file\n"
