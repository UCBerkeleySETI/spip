...
int main(int argc, char *argv[]) try
{
  ...
  if (verbose)
    cerr << "uwb_dbstats: creating DataBlockStats with " << key << endl;
  ...
  if (verbose)
    cerr << "uwb_dbstats: reading config from " << config_file << endl;
  ...
  if (verbose)
    cerr << "uwb_dbstats: configuring using fixed config" << endl;
  ...
  if (control_port > 0)
  {
    cerr << "uwb_dbstats: start_control_thread (" << control_port << ")" << endl;
    ...
    {
      if (verbose)
        cerr << "uwb_dbstats: monitor()" << endl;
      keep_monitoring = dbstats->monitor(stats_dir, stream);
      if (verbose)
        cerr << "uwb_dbstats: monitor returned" << endl;
    }
  }
  else
  {
...
void usage() 
{
  cout << "uwb_dbstats [options] config\n"
