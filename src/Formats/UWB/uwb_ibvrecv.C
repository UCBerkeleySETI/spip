...
int main(int argc, char *argv[])
{
  ...
  try 
  {
    ...
    ibvrecv->set_format (format);

    if (verbose)
      cerr << "uwb_ibvrecv: Loading configuration from " << argv[optind] << endl;

    // config file for this data stream
    config.load_from_file (argv[optind]);

    if (ibvrecv->verbose)
      cerr << "uwb_ibvrecv: configuring using fixed config" << endl;
    ibvrecv->configure (config.raw());

    if (ibvrecv->verbose)
      cerr << "uwb_ibvrecv: allocating runtime resources" << endl;
    ibvrecv->prepare ();

    if (ibvrecv->verbose)
      cerr << "uwb_ibvrecv: starting stats thread" << endl;
    pthread_t stats_thread_id;
    int rval = pthread_create (&stats_thread_id, 0, stats_thread, (void *) ibvrecv);
    if (rval != 0)
    {
      cerr << "uwb_ibvrecv: failed to start stats thread" << endl;
      return (EXIT_FAILURE);
    }

    if (ibvrecv->verbose)
      cerr << "uwb_ibvrecv: receiving" << endl;
    ibvrecv->receive ();

    quit_threads = 1;

    if (ibvrecv->verbose)
      cerr << "uwb_ibvrecv: joining stats_thread" << endl;
    void * result;
  ...
  {
    cerr << "uwb_ibvrecv: ERROR: " << exc.what() << endl;
    return -1;
  }
...
void usage() 
{
  cout << "uwb_ibvrecv [options] config\n"
...
void * stats_thread (void * arg)
{
  spip::IBVReceiver * recv = (spip::IBVReceiver *) arg;
