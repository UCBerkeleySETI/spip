...
int main(int argc, char *argv[])
{
  ...
  try 
  {
    ...
    udprecv->set_format (new spip::UDPFormatVDIF());

    if (verbose)
      cerr << "vdif_udprecv: Loading configuration from " << argv[optind] << endl;

    // config file for this data stream
    config.load_from_file (argv[optind]);

    if (udprecv->verbose)
      cerr << "vdif_udprecv: configuring using fixed config" << endl;
    udprecv->configure (config.raw());

    if (udprecv->verbose)
      cerr << "vdif_udprecv: allocating runtime resources" << endl;
    udprecv->prepare ();
    ...
    if (udprecv->verbose)
      cerr << "vdif_udprecv: receiving" << endl;
    udprecv->receive ();

    quit_threads = 1;

    if (udprecv->verbose)
      cerr << "vdif_udprecv: joining stats_thread" << endl;
    void * result;
  ...
  {
    cerr << "vdif_udprecv: ERROR: " << exc.what() << endl;
    return -1;
  }
...
void usage() 
{
  cout << "vdif_udprecv [options] config\n"
