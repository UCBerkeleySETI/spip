...
int main(int argc, char *argv[]) try
{
  ...
  spip::ExampleReadWriteBlockClient * op;

  op = new spip::ExampleReadWriteBlockClient (read_key.c_str(),
                                              write_key.c_str());
  if (verbose)
    cerr << "example_readwriteblockclient: main()" << endl;
  try
  {
    op->main();
  }
  ...
  {
    cerr << "example_readwriteblockclient: ERROR: " << exc.what() << endl;
    return -1;
  }
...
void usage()
{
  cout << "example_readwriteblockclient [options] input out_key\n"
