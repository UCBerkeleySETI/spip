...
spip::IBVReceiverMerge2::IBVReceiverMerge2 (boost::asio::io_service& io_service)
...
spip::IBVReceiverMerge2::~IBVReceiverMerge2()
{
#ifdef _DEBUG
  cerr << "spip::IBVReceiverMerge2::~IBVReceiverMerge2" << endl;
#endif
...
int spip::IBVReceiverMerge2::configure (const char * config_str)
{
  // save the config for use on the first open block
  config.load_from_str (config_str);

  if (verbose)
    cerr << "spip::IBVReceiverMerge2::configure" << endl;
...
void spip::IBVReceiverMerge2::set_formats (spip::UDPFormat * fmt1, spip::UDPFormat * fmt2)
{
  if (formats[0])
    delete formats[0];
  formats[0] = fmt1;
...
void spip::IBVReceiverMerge2::start_control_thread (int port)
{
  control_port = port;
  pthread_create (&control_thread_id, NULL, control_thread_wrapper, this);
}

void spip::IBVReceiverMerge2::stop_control_thread ()
{
  control_cmd = Quit;
  void * result;
...
void spip::IBVReceiverMerge2::set_control_cmd (spip::ControlCmd cmd)
{
  pthread_mutex_lock (&mutex_db);
#ifdef _DEBUG
  cerr << "spip::IBVReceiverMerge2::set_control_cmd cmd=" << cmd << endl;
#endif
  control_cmd = cmd;
...
void spip::IBVReceiverMerge2::control_thread()
{
#ifdef _DEBUG
  cerr << "spip::IBVReceiverMerge2::control_thread starting" << endl;
#endif
...
#ifdef _DEBUG
  cerr << "spip::IBVReceiverMerge2::control_thread creating TCPSocketServer" << endl;
#endif
...
#ifdef _DEBUG
  cerr << "spip::IBVReceiverMerge2::control_thread exiting" << endl;
#endif
}

bool spip::IBVReceiverMerge2::open ()
{
  if (control_cmd == Stop)
  {
    return false;
  ...
  {
    cerr << "No SOURCE in header1, using J0437-4715" << endl;
  ...
  {
    cerr << "No SOURCE in header2, using J0437-4715" << endl;
    if (header2.set ("SOURCE", "%s", "J0437-4715") < 0)
      throw invalid_argument ("failed to write SOURCE to header2");
  }

#ifdef _DEBUG
  cerr << "spip::IBVReceiverMerge2::open formats->prepare()" << endl;
#endif
...
void spip::IBVReceiverMerge2::open (const char * header1_str, const char * header2_str)
{
  if (verbose)
    cerr << "spip::IBVReceiverMerge2::open()" << endl;
}

void spip::IBVReceiverMerge2::close ()
{
  if (verbose)
    cerr << "spip::IBVReceiverMerge2::close()" << endl;
...
void spip::IBVReceiverMerge2::send_terminal_packets()
{
#ifdef _DEBUG
  cerr << "spip::IBVReceiverMerge2::send_terminal_packets()" << endl;
#endif
...
void spip::IBVReceiverMerge2::start_threads (int c1, int c2)
{
...
void spip::IBVReceiverMerge2::join_threads ()
{
  if (verbose)
    cerr <<" spip::IBVReceiverMerge2::join_threads()" << endl;
  void * result;
...
bool spip::IBVReceiverMerge2::datablock_thread ()
{
#ifdef _DEBUG
  cerr << "spip::IBVReceiverMerge2::datablock_thread starting" << endl;
#endif
  pthread_mutex_lock (&mutex_db);
...
#ifdef _DEBUG
  cerr << "spip::IBVReceiverMerge2::datablock_thread locked mutexes" << endl;
#endif

  // i is sub-band, j is pol
  for (unsigned i=0; i<2; i++)
  {
    curr_blocks[i] = (char *) malloc (block_bufsz);
    next_blocks[i] = (char *) malloc (block_bufsz);
    last_blocks[i] = (char *) malloc (block_bufsz);
...
#ifdef _DEBUG
  cerr << "spip::IBVReceiverMerge2::datablock allocated memory" << endl;
#endif
...
#ifdef _DEBUG
    cerr << "spip::IBVReceiverMerge2::datablock control_cmd  != None" << endl;
#endif
...
#ifdef _DEBUG
    cerr << "spip::IBVReceiverMerge2::datablock opened buffers for ibuf=" << ibuf << endl;
#endif
  }
...
#ifdef _DEBUG
    cerr << "spip::IBVReceiverMerge2::datablock_thread received "
...
#ifdef _DEBUG
  cerr << "spip::IBVReceiverMerge2::datablock signaled recv " << ibuf << endl;
#endif
  ...
  while (control_state == Active)
  {
    ...
    // bother threads have now completed the block
    ibuf++;
    ...
    {
#ifdef _DEBUG
      cerr << "spip::IBVReceiverMerge2::datablock checking buffer " << ibuf 
           << " [" << full[0] << "," << full[1] << "]" << endl;
#endif
      pthread_cond_wait (&cond_db, &mutex_db);
    }
...
#ifdef _DEBUG
    cerr << "spip::IBVReceiverMerge2::datablock filled buffer " << ibuf << endl;
#endif
    ...
    {
      tmp_blocks[0] = curr_blocks[0];
      tmp_blocks[1] = curr_blocks[1];
      ...
      next_blocks[0] = tmp_blocks[0];
      next_blocks[1] = tmp_blocks[1];
...
//#ifdef _DEBUG
  cerr << "spip::IBVReceiverMerge2::data_block exiting" << endl;
//#endif

  return true;
}

bool spip::IBVReceiverMerge2::receive_thread (int p)
{
#ifdef _DEBUG
  cerr << "spip::IBVReceiverMerge2::receive_thread[" << p << "] starting" << endl;
#endif
#ifdef HAVE_HWLOC
  ...
  // block accounting 
  const int64_t data_bufsz = (block_bufsz + block_bufsz) / 2;
...
#ifdef _DEBUG
  cerr << "spip::IBVReceiverMerge2::receive_thread[" << p << "] trying to lock mutex" << endl;
#endif
  ...
  if (verbose)
    cerr << "spip::IBVReceiverMerge2::receive["<<p<<"] control_state != Idle" << endl;
  ...
  while (control_states[p] == Active)
  {
    ...
    {
#ifdef _DEBUG
      cerr << "spip::IBVReceiverMerge2::receive["<<p<<"] waiting for empty " << ibuf << " [" << full[0] << ", " << full[1] << "]" << endl;
#endif
      pthread_cond_wait (cond_recv, mutex_recv);
    }
    ...
    if (control_states[p] == Active)
    {
...
#ifdef _DEBUG
      if (p == 1)
        cerr << "spip::IBVReceiverMerge2::receive["<<p<<"] filling buffer " 
      ...
      while (!filled_curr_buffer && spip::IBVQueue::keep_receiving)
      {
...
#ifdef _DEBUG
            if (p == 0)
              cerr << "spip::IBVReceiverMerge2::receive["<<p<<"] close_block "
        ...
        {
#ifdef _TRACE
          cerr << "spip::IBVReceiverMerge2::receive no packet at socket" << endl;
#endif
        }
        else
        {
...
#ifdef _DEBUG
      cerr << "spip::IBVReceiverMerge2::receive["<<p<<"] filled buffer " << ibuf << endl; 
#endif
      ibuf++;
      pthread_cond_signal (&cond_db);
      pthread_mutex_unlock (&mutex_db);
...
//#ifdef _DEBUG
  cerr << "spip::IBVReceiverMerge2::receive["<<p<<"] exiting" << endl;
//#endif
 ...
 */
void spip::IBVReceiverMerge2::stats_thread()
{
...
#ifdef _DEBUG
  cerr << "spip::IBVReceiverMerge2::stats_thread starting polling" << endl;
#endif
...
/*
      if (report_count % 20 == 0)
        for (unsigned i=0; i<formats[1]->packets_per_stream.size(); i++)
        {
          fprintf (stderr, "%u %lu\n", i, formats[1]->packets_per_stream[i]);
          formats[1]->packets_per_stream[i] = 0;
        }
*/
      sleep (1);
      report_count++;
...
#ifdef _DEBUG
  cerr << "spip::IBVReceiverMerge2::stats_thread exiting";
#endif
}
