...
spip::UDPReceiveMergeDBs::UDPReceiveMergeDBs (vector<string> keys)
{
  for (auto key = keys.begin(); key != keys.end(); ++key)
  {
    dbs.push_back(new DataBlockWrite ((*key).c_str()));
  }

  blocks.resize(dbs.size());
  overflows.resize(dbs.size());
  overflow_lastbytes.resize(dbs.size());
...
spip::UDPReceiveMergeDBs::~UDPReceiveMergeDBs()
{
#ifdef _DEBUG
  cerr << "spip::UDPReceiveMergeDBs::~UDPReceiveMergeDBs" << endl;
#endif
...
int spip::UDPReceiveMergeDBs::configure (const char * config_str)
{
...
void spip::UDPReceiveMergeDBs::set_formats (spip::UDPFormat * fmt1, spip::UDPFormat * fmt2)
{
  if (formats[0])
    delete formats[0];
  formats[0] = fmt1;
...
void spip::UDPReceiveMergeDBs::start_control_thread (int port)
{
  control_port = port;
  pthread_create (&control_thread_id, NULL, control_thread_wrapper, this);
}

void spip::UDPReceiveMergeDBs::stop_control_thread ()
{
  control_cmd = Quit;
  void * result;
...
void spip::UDPReceiveMergeDBs::set_control_cmd (spip::ControlCmd cmd)
{
...
void spip::UDPReceiveMergeDBs::control_thread()
{
#ifdef _DEBUG
  cerr << "spip::UDPReceiveMergeDBs::control_thread starting" << endl;
#endif
...
#ifdef _DEBUG
  cerr << "spip::UDPReceiveMergeDBs::control_thread creating TCPSocketServer" << endl;
#endif
...
#ifdef _DEBUG
  cerr << "spip::UDPReceiveMergeDBs::control_thread exiting" << endl;
#endif
}

bool spip::UDPReceiveMergeDBs::open ()
{
  if (verbose > 1)
    cerr << "spip::UDPReceiveMergeDBs::open()" << endl;
  ...
  if (header.get ("UTC_START", "%s", buffer) == -1)
  {
    ...
    cerr << "spip::UDPReceiveMergeDBs::open UTC_START=" << buffer << endl;
  ...
  if (verbose > 1)
    cerr << "spip::UDPReceiveMergeDBs::open preparing formats" << endl;
...
uint64_t spip::UDPReceiveMergeDBs::get_data_bufsz()
{
  uint64_t total_size = 0;
  for (auto db = dbs.begin(); db != dbs.end(); ++db)
  {
    total_size +=  (*db)->get_data_bufsz();
  }
  return total_size;
}

// write the ascii header to the datablock, then
void spip::UDPReceiveMergeDBs::open (const char * header_str)
{
  for (auto db = dbs.begin(); db != dbs.end(); ++db)
  {
    ...
    // write the header TODO customize the header
    (*db)->write_header (header_str);
  }
}

void spip::UDPReceiveMergeDBs::close ()
{
  if (verbose)
    cerr << "spip::UDPReceiveMergeDBs::close()" << endl;
  ...
  for (auto db = dbs.begin(); db != dbs.end(); ++db)
  {
    ...
    {
      if (verbose)
        cerr << "spip::UDPReceiveMergeDBs::close db->close_block(" << (*db)->get_data_bufsz() << ")" << endl;
...
void spip::UDPReceiveMergeDBs::start_threads (int c1, int c2)
{
...
void spip::UDPReceiveMergeDBs::join_threads ()
{
  void * result;
...
bool spip::UDPReceiveMergeDBs::datablock_thread ()
{
  pthread_mutex_lock (&mutex_db);
  ...
  if (control_cmd == Start)
  {
    ...
    {
      // open the data block for writing
      blocks[i] = (char *) (dbs[i]->open_block());
    }
    ...
    for (auto oflb=overflow_lastbytes.begin(); oflb != overflow_lastbytes.end(); oflb++)
      (*oflb)[0] = (*oflb)[1] = 0;

#ifdef _DEBUG
    cerr << "spip::UDPReceiveMergeDBs::datablock opened buffer " << ibuf << endl;
#endif
  }
...
#ifdef _DEBUG
    cerr << "spip::UDPReceiveMergeDBs::datablock_thread received "
...
#ifdef _DEBUG
  cerr << "spip::UDPReceiveMergeDBs::datablock signaled recv " << ibuf << endl;
#endif
  ...
  while (control_state == Active)
  {
    ...
    {
#ifdef _DEBUG
      cerr << "spip::UDPReceiveMergeDBs::datablock checking buffer " << ibuf 
           << " [" << full[0] << "," << full[1] << "]" << endl;
#endif
      pthread_cond_wait (&cond_db, &mutex_db);
    }
...
#ifdef _DEBUG
    cerr << "spip::UDPReceiveMergeDBs::datablock filled buffer " << ibuf << endl;
#endif
    ...
    else
    {
      ...
      for (unsigned i=0; i<overflows.size(); i++)
      {
        char * overflow = overflows[i];  
        int64_t * lbs = overflow_lastbytes[i];
        // copy any overflowed data into place
        overflow_lastbyte = std::max (lbs[0], lbs[1]);
        if (overflow_lastbyte > 0)
        {
#ifdef _DEBUG
          cerr << "spip::UDPReceiveMergeDBs::data_block overflow saved " << overflow_lastbyte << " bytes" << endl;
#endif
...
#ifdef _DEBUG
  cerr << "spip::UDPReceiveMergeDBs::data_block exiting" << endl;
#endif

  return true;
}

bool spip::UDPReceiveMergeDBs::receive_thread (int p)
{
#ifdef HAVE_HWLOC
  ...
  // block accounting 
  const int64_t data_bufsz = this->get_data_bufsz();
  ...
  int64_t byte_offset;
  const uint64_t pol_span = chunk_size / 2;
  const uint64_t pol_offset = p * pol_span;
  const uint64_t subband_span = pol_span / dbs.size();
...
#ifdef _DEBUG
  cerr << "spip::UDPReceiveMergeDBs::receive["<<p<<"] control_states[" << p << "] now != Idle" << endl;
#endif
  ...
  while (control_states[p] == Active)
  {
    ...
    {
#ifdef _DEBUG
      cerr << "spip::UDPReceiveMergeDBs::receive["<<p<<"] waiting for empty " << ibuf << " [" << full[0] << ", " << full[1] << "]" << endl;
#endif
      pthread_cond_wait (cond_recv, mutex_recv);
    }
...
#ifdef _DEBUG
      if (p == 1)
        cerr << "spip::UDPReceiveMergeDBs::receive["<<p<<"] filling buffer " 
      ...
      while (!filled_this_buffer && keep_receiving)
      {
        if (vma_api)
        {
          ...
          while (!have_packet && keep_receiving)
          {
            ...
            {
              cerr << "spip::UDPReceiveMergeDBs::receive error expected " << sock_bufsz
        ...
        {
          // determine output data block ID
          output_block = (byte_offset % chunk_size) / subband_span;
...
#ifdef _DEBUG
          if (p == 1)
            cerr << "spip::UDPReceiveMergeDBs::receive["<<p<<"] close_block "
  ...
  #ifdef _DEBUG
      cerr << "spip::UDPReceiveMergeDBs::receive["<<p<<"] filled buffer " << ibuf << endl; 
  #endif
      ibuf++;
      pthread_cond_signal (&cond_db);
      pthread_mutex_unlock (&mutex_db);
...
#ifdef _DEBUG
  cerr << "spip::UDPReceiveMergeDBs::receive["<<p<<"] exiting" << endl;
#endif
 ...
 */
void spip::UDPReceiveMergeDBs::stats_thread()
{
...
#ifdef _DEBUG
  cerr << "spip::UDPReceiveMergeDBs::stats_thread starting polling" << endl;
#endif
...
#ifdef _DEBUG
  cerr << "spip::UDPReceiveMergeDBs::stats_thread exiting";
#endif
}
