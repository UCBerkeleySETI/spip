...
__global__ void IntegrationBinned_TSPF_TSCR_Kernel (float * in, float * buf, float * out, 
...
void spip::IntegrationBinnedCUDA::transform_TSPF_to_TSPFB ()
{
  ...
  if (verbose)
  {
    cerr << "spip::IntegrationBinnedCUDA::transform_TSPF_to_TSPF nchan=" << nchan << " nsignal=" << nsignal << " npol=" << npol << " ndat=" << ndat << endl;
    cerr << "spip::IntegrationBinnedCUDA::transform_TSPF_to_TSPF chan_dec=" << chan_dec << " dat_dec=" << dat_dec << endl;
    cerr << "spip::IntegrationBinnedCUDA::transform_TSPF_to_TSPF buffer->get_size()=" << buffer->get_size() << endl;
  }
  ...
  if (chan_dec > 1)
  {
    ...
    if (verbose) 
    {
      cerr << "spip::IntegrationBinnedCUDA::transform_TSPF_to_TSPF IntegrationBinned_TSPF_FSCR_Kernel" << endl;
  ...
  if (dat_dec > 1)
  {
    ...
    if (verbose) 
    {
      cerr << "spip::IntegrationBinnedCUDA::transform_TSPF_to_TSPF IntegrationBinned_TSPF_TSCR_Kernel buffer_idat=" << buffer_idat << " ndat=" << ndat << endl;
      cerr << "spip::IntegrationBinnedCUDA::transform_TSPF_to_TSPF IntegrationBinned_TSPF_TSCR_Kernel nchan_work=" << nchan_work << endl;
    }

    // then perform Tscrunching
    IntegrationBinned_TSPF_TSCR_Kernel<<<blocks, nthread, 0, stream>>>(in, buf, out, ndat, nchan, buffer_idat, dat_dec, sigpol_stride, dat_stride);
