...
class continuum extends spip_webpage
{
  ...
  function continuum()
  {
    ...
    for ($istream=0; $istream<$this->config["NUM_STREAM"]; $istream++)
    {
      ...
      $this->streams[$istream] = array("beam_name" => $beam_name, "host" => $host, "subband" => $subband, "freq" => $freq);
    }
...
?>
    <script type='text/javascript'>
      ...
      function handle_continuum_request(t_xml_request)
      {
              ...
              for (i=0; i<beams.length; i++)
              {
                ...
                //if (active == "True")
                {
                  ...
                  for (j=0; j<plots.length; j++)
                  {
                    ...
                    {
                      url = "/spip/continuum/index.php?update=true&beam_name="+beam_name+"&type=plot&pol=0&plot="+plot_type+"&res=lo&ts="+plot_timestamp;
                      //alert (url);
                      document.getElementById(plot_id).src = url;
                      document.getElementById(plot_ts).value = plot_timestamp;

                      url = "/spip/continuum/index.php?update=true&beam_name="+beam_name+"&type=plot&pol=0&plot="+plot_type+"&res=hi&ts="+plot_timestamp;
      ...
      function continuum_request() 
      {
        ...
        {
          handle_continuum_request(t_xml_request)
        };
...
<?php
  ...
  function printUpdateHTML($get)
  {
    ...
    $xml = XML_DEFINITION;
    $xml .= "<continuum_update>";

    $xml_req  = XML_DEFINITION;
    $xml_req .= "<repack_request>";
    $xml_req .= "<requestor>continuum page</requestor>";
    ...
    foreach ($this->beams as $ibeam => $beam)
    {
      ...
      {
        $host = $beam["host"];
        $port = $this->config["BEAM_REPACK_CONTINUUM_PORT"] + $ibeam;
      }
      else
      {
  ...
  function renderImage($get)
  {
    ...
    if (strpos($get["plot"], "bandpass") === 0)
    {
      ...
      $port = $this->config["BEAM_REPACK_CONTINUUM_PORT"];
  ...
  function renderPlotTable ($beam, $stream)
  {
    ...
    foreach ($this->streams as $istream => $stream)
    {
      ...
      echo   "<td>";
      echo     "<a id='".$beam."_bandpass".$freq."_link'>";
      echo       "<img id='".$beam."_bandpass".$freq."' ".$full_img_params."/>";
      echo     "</a>";
      echo     "<input type='hidden' id='".$beam."_bandpass".$freq."_ts' value='not set'/>";
      echo   "</td>\n";
